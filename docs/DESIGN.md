...
- 終了後: スコアサマリ（n問中m問正解、間違えた単語へのリンク）。
  - **満点時の祝福演出**: 紙吹雪エフェクト（`Confetti` コンポーネント、テーマトークンの配色、`prefers-reduced-motion` で非表示）+ ロキのバストアップイラスト（`LokiPortrait`。狐面を手に持った男性のシルエット、ホーム画面の `FoxIcon` と同じ浮遊・発光演出）+ ロキ人格（§4.2）のトーンで書いた祝福メッセージ（複数パターンからランダム表示）。結果画面遷移時（`handleNext` で `stage` を `'result'` にする瞬間）に1回だけ抽選する。
