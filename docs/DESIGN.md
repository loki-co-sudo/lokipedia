...
- 終了後: スコアサマリ（n問中m問正解、間違えた単語へのリンク）。
  - **満点時の祝福演出**: 紙吹雪エフェクト（`Confetti` コンポーネント、テーマトークンの配色、`prefers-reduced-motion` で非表示）+ ロキの立ち絵（`src/assets/loki-portrait.webp`。狐面を持った男性のイラスト、管理者提供）を円形メダリオン風に配置（縁取り + 発光ハロー、ホーム画面の `FoxIcon` と同系統の浮遊演出）+ ロキ人格（§4.2）のトーンで書いた祝福メッセージ（複数パターンからランダム表示）。結果画面遷移時（`handleNext` で `stage` を `'result'` にする瞬間）に1回だけ抽選する。
