...
- [ ] `src/lib/gemini.ts`: DESIGN.md §4 の仕様で実装。`responseSchema` を必ず使う。既存タグ一覧を引数で受け取りプロンプトに含める。
- [ ] ホーム画面: 入力欄 + タグチップ入力 + 「AIで生成」→ ローディング → プレビューカード（Markdown レンダリング + クイズ全文とその解説）。
- [ ] プレビューから「辞書に登録」で `createWordWithQuiz()` 実行 → 成功トースト → 辞書詳細へ遷移。
- [ ] 「再生成」ボタン（同じ入力で引き直し）。
- [ ] 未ログイン時: 生成ボタン無効 + 「管理者のみ利用できます」表示。Gemini キー未設定時: 設定画面への導線。
- [ ] エラー時（ネットワーク・スキーマ不一致・4択でない等）は日本語でエラー表示。自動リトライしない。

### 受け入れ条件
- 実際の Gemini API キーで「PWA」を生成 → 登録 → Supabase のテーブルに行が入る（管理者が実キーで確認する。エージェントはモックでの動作確認まで）。
- 選択肢が4つでない応答を意図的に与えた場合（テストで再現）、保存されずエラー表示になる。
...
- [ ] 一覧: カードグリッド、term / タグチップ / definition 抜粋（プレーンテキスト化して2行）。
- [ ] タグフィルタ（複数選択 AND）+ フリーテキスト絞り込み（term と definition を対象）。
- [ ] 詳細: definition の Markdown レンダリング（react-markdown、HTML 無効のまま）、クイズ一覧、この端末の解答履歴（正答率・直近解答）。
- [ ] 管理者のみ表示: タグの追加/削除（`updateWordTags`）、単語削除（確認ダイアログ必須）、「クイズを追加生成」。
- [ ] オフライン時も IndexedDB キャッシュから一覧・詳細が表示できる。

### 受け入れ条件
- 未ログイン（シークレットウィンドウ）で一覧・詳細・フィルタが動作する。
- タグ操作・削除ボタンが未ログイン時に表示されない。
