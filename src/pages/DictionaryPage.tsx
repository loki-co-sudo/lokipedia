    ...
    <div className="space-y-4">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-xl border border-app-border bg-app-surface py-2.5 pl-9 pr-4 outline-none focus:border-app-accent"
        />
      </div>
        ...
        {(['latest', 'kana'] as const).map((opt) => (
          <button
            ...
            className={`rounded-xl px-4 py-2 text-sm font-semibold ${
              sort === opt ? 'bg-app-accent text-app-on-accent' : 'bg-app-surface-2 text-app-text-muted'
            }`}
          >
            {opt === 'latest' ? '新着順' : '50音順'}
          </button>
      ...
      {loading && (
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="space-y-2 rounded-xl border border-app-border bg-app-surface p-4">
