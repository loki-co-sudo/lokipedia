  ...
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-xl border border-app-border bg-app-surface px-3 py-2 focus-within:border-app-accent">
      {value.map((tag) => (
        <span
          key={tag}
          className="flex max-w-full min-w-0 items-center gap-1 rounded-full bg-app-accent/10 px-3 py-1 text-xs font-medium text-app-accent"
        >
          <span className="break-words">{tag}</span>
          <button type="button" onClick={() => removeTag(tag)} aria-label={`${tag}を削除`} className="shrink-0">
            <X className="h-3 w-3" />
          </button>
        </span>
      ))}
